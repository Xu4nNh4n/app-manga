...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Widget _buildStoriesTab(bool isDark) {
    return Scaffold(
      ...
      body: StreamBuilder<List<Story>>(
        stream: _controller.watchStories(),
            ...
            itemBuilder: (context, index) {
              ...
              return Container(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(
                    '${story.chapterCount} chương • ${story.views} views • ⭐ ${story.rating}',
                    style: TextStyle(
                  ...
                  trailing: PopupMenuButton<String>(
                    onSelected: (v) {
                      ...
                      if (v == 'add_chapter') {
                        _showAddChapterDialog(story, isDark);
                      }
  ...
  void _showAddStoryDialog(bool isDark) {
    ...
    final fc = TextEditingController(text: '3'),
        cc = TextEditingController(text: '5');
    _showStoryForm(
      isDark,
      'Thêm Truyện Mới',
      t,
      ...
      'Thêm',
      () async {
        final form = AdminStoryFormData.fromText(
          title: t.text,
          author: a.text,
          description: d.text,
          coverImage: c.text,
          genres: g.text,
          freeChapters: fc.text,
          coinPerChapter: cc.text,
        );
        if (!form.isValid) return;
        Navigator.pop(context);
        await _controller.addStory(form);
  ...
  void _showEditStoryDialog(Story s, bool isDark) {
    final t = TextEditingController(text: s.title),
        a = TextEditingController(text: s.author);
    final d = TextEditingController(text: s.description),
        c = TextEditingController(text: s.coverImage);
    final g = TextEditingController(text: s.genres.join(', '));
    final fc = TextEditingController(text: '${s.freeChapters}'),
        cc = TextEditingController(text: '${s.coinPerChapter}');
    _showStoryForm(
      isDark,
      'Sửa Truyện',
      t,
      ...
      () async {
        ...
        if (!form.isValid) return;
        Navigator.pop(context);
        await _controller.updateStory(s, form);
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              _field(t, 'Tên truyện', isDark),
              _field(a, 'Tác giả', isDark),
              _field(d, 'Mô tả', isDark, maxLines: 3),
              _field(c, 'URL ảnh bìa', isDark),
              _field(g, 'Thể loại (cách nhau bởi dấu ,)', isDark),
              _field(fc, 'Số chương miễn phí', isDark),
              _field(cc, 'Giá xu / chương', isDark),
            ],
          ),
        ),
        actions: [
  ...
  void _showAddChapterDialog(Story story, bool isDark) {
    final t = TextEditingController(),
        n = TextEditingController(text: '${story.chapterCount + 1}'),
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              _field(n, 'Số chương', isDark),
              _field(t, 'Tiêu đề chương', isDark),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              final form = AdminChapterFormData.fromText(
                chapterNumber: n.text,
                title: t.text,
                pages: p.text,
              );
              if (!form.isValid) return;
              Navigator.pop(ctx);
              await _controller.addChapter(story, form);
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              await _controller.deleteStory(story);
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              await _controller.addCategory(ctrl.text);
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              if (ctrl.text.trim().isEmpty) return;
              Navigator.pop(ctx);
              await _controller.updateCategory(cat, ctrl.text);
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              await _controller.deleteCategory(cat);
